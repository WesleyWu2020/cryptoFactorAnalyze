...
from util_factor import (
    ...
    # 数据预处理函数
    prepare_robust_data, prepare_xgboost_data_fast,
    # 特征选择函数
...
def create_xgboost_prediction_factor(window=20, rebalance_period=5, train_epochs=3000,
                                    ...
                                    train_window_days=360,       # 训练窗口长度（天数）
    ...
    print(f"🚀 开始构建XGBoost预测因子 ({opt_type}, {ortho_type}, {feature_type}, {training_type}, {selection_type}, {preprocessing_type}, {target_type}, {reg_type}, {training_mode})")
    ...
    for window_idx, (train_data, val_data, test_data, window_info) in enumerate(rolling_windows):
        ...
        if use_cached_window:
            ...
            corr_save_path = reports_dir / f"xgb_features_correlation_w{window}_r{rebalance_period}_w{window_idx}.png"
            analyze_selected_features_correlation(
                X_train_selected, X_train_selected.shape[1],
                title=f"XGBoost选定特征相关性分析 (w{window}_r{rebalance_period}_window{window_idx})",
                save_path=str(corr_save_path),
                feature_names=all_feature_names[:X_train_selected.shape[1]]  # 只取选定特征的数量
            )

            # 特征重要性分析
            importance_save_path = reports_dir / f"xgb_features_importance_w{window}_r{rebalance_period}_w{window_idx}.png"
            analyze_feature_importance(
                X_train_selected, y_train, X_train_selected.shape[1],
                title=f"XGBoost特征重要性分析 (w{window}_r{rebalance_period}_window{window_idx})",
                save_path=str(importance_save_path),
                feature_names=all_feature_names[:X_train_selected.shape[1]]  # 只取选定特征的数量
            )

        # 标准化 - 已移除RobustScaler，因为已经在prepare_xgboost_data_fast中按日期进行了横截面标准化
        # 横截面标准化已经将每个日期的特征标准化为均值0、标准差1，不需要再做全局标准化
        # 这样可以保持不同季度间的一致性，避免因训练集统计量导致的分布偏移
        print("✅ 特征已在prepare_xgboost_data_fast中按日期进行横截面标准化，跳过RobustScaler")
        ...
        if use_cached_window:
            booster = xgb.Booster()
            booster.load_model(str(window_model_path))
            model = booster
            print("✅ 已加载缓存模型与预处理器")
        else:
    ...
    if len(final_df) > 0:
        out_path = save_factor_df(final_df, file_prefix=f"xgboost_{window}d_rebalance{rebalance_period}d_")
