...
def create_alpha16_factor(covariance_window=5, rebalance_period=3, normalization_method='relative_price'):
    """
    创建 Alpha 16 因子 (币圈7×24h优化版)
    ...
    原理:
    Alpha 16 = (-1 * rank(covariance(rank(normalized_high), rank(normalized_volume), 5)))
    ...
    该因子通过分析归一化后最高价排名与交易量排名之间的协方差，寻找可能的价格反转机会
    特别关注那些价格高点与交易量排名关系异常的股票，这可能表明市场参与者对这些股票的估值存在分歧或投机活动
    """
    print(f"开始构建 Alpha 16 因子 (币圈7×24h优化版)...")
    ...
    # 创建因子计算函数
    def calculate_alpha16_factor(group):
        ...
        if normalization_method == 'relative_price':
            ...
            # 方法3: 使用价格变化率
            group['normalized_high'] = group['high'].pct_change(periods=5)  # 5日价格变化率
        ...
        # 只保留需要的列
        result = group[['date', 'symbol', 'normalized_high', 'normalized_volume', 'future_ret']].copy()
    ...
    # 按交易对分组计算
    print("计算 Alpha 16 因子基础数据...")
    result_dfs = []
    
    # 使用tqdm显示进度
    for symbol, group in tqdm(df.groupby('symbol')):
        factor_result = calculate_alpha16_factor(group)
    ...
    def calculate_covariance(group):
        ...
        # 计算过去N天归一化最高价与归一化交易量的协方差
        group['covariance'] = group['normalized_high'].rolling(window=covariance_window, min_periods=covariance_window).cov(group['normalized_volume'])
    ...
    def calculate_final_factor(df):
        ...
        df['alpha16_raw'] = -1 * df['covariance_rank']
    ...
    def winsorize_by_date(df):
        df = df.copy()
        mean = df['alpha16_raw'].mean()
        std = df['alpha16_raw'].std()
        df['alpha16_raw'] = df['alpha16_raw'].clip(lower=mean-3*std, upper=mean+3*std)
        return df
    ...
    def normalize_by_date(df):
        """使用排序归一化到-1到1之间"""
        df = df.copy()
        rank_pct = df['alpha16_raw'].rank(pct=True)
        df['alpha16_factor'] = 2 * (rank_pct - 0.5)  # 将[0,1]映射到[-1,1]
        return df
    ...
    output_path = os.path.join(output_dir, f"alpha16_covariance_rank_cov{covariance_window}d_{normalization_method}_rebalance{rebalance_period}d_{today}.csv")
...
if __name__ == "__main__":
    ...
    create_alpha16_factor(covariance_window=10, rebalance_period=10, normalization_method='market_cap')
    
    # 策略2: 短期策略 + 价格变化率归一化
    # create_alpha16_factor(covariance_window=3, rebalance_period=2, normalization_method='price_change')
    
    # 策略3: 中期策略 + 市值加权归一化
    # create_alpha16_factor(covariance_window=7, rebalance_period=5, normalization_method='market_cap')
    
    # 策略4: 超短期策略 + 相对价格归一化
    # create_alpha16_factor(covariance_window=2, rebalance_period=1, normalization_method='relative_price')
    
    # 策略5: 长期策略 + 相对价格归一化
    # create_alpha16_factor(covariance_window=10, rebalance_period=7, normalization_method='relative_price')
