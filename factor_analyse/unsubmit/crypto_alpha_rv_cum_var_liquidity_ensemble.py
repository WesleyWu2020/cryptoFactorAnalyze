"""crypto_alpha_rv_cum_var_liquidity_ensemble 因子（factor_common 日频契约版）。

原始定义（已是日频语义，分钟版仅做 resample）：
    factor = -0.5 * rank40(mean10(rv_ratio_5_20 风格残差))
             + 1.0 * var5(cum_return) + 1.0 * var10(cum_return)
             + 0.168 * |rank10(dollar_volume_rank)|
...
META = {
    "factor_name": "crypto_alpha_rv_cum_var_liquidity_ensemble",
    "author": "wesleywu",
    "level": "daily",
    "category": "ensemble",
    "description": "-0.5*rank40(mean10(rv_ratio_resid)) + var5(cumret) + var10(cumret) + 0.168*|rank10(dollar_rank)|",
}
...
SETTING = {
    ...
    "params": {
        ...
        "mean_window_days": 10,
        "rank_window_days": 40,
        "var_short_window_days": 5,
        "var_long_window_days": 10,
        "dollar_rank_days": 10,
    },
...
def _rolling_var(frame: pd.DataFrame, window: int) -> pd.DataFrame:
    return frame.rolling(window, min_periods=window).var(ddof=1)
...
def calc_factor(data_ctx: dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    component_2 = _rolling_var(cum_return, params["var_short_window_days"])
    component_3 = _rolling_var(cum_return, params["var_long_window_days"])
    component_4 = _rolling_rank(dollar_rank, params["dollar_rank_days"]).abs()

    factor = -0.5 * component_1 + 1.0 * component_2 + 1.0 * component_3 + 0.168 * component_4
