"""crypto_alpha_volume_surge_trend_decay_h10 因子（factor_common 日频契约版）。
...
原始定义：
    ...
    事件值 = dir × align × (0.5 + 0.5×strength)，仅触发日赋值，
    其后按半衰期 10d 指数衰减（event_decay），无事件不新开仓。
...
META = {
    "factor_name": "crypto_alpha_volume_surge_trend_decay_h10",
    "author": "wesleywu",
    "level": "daily",
    "category": "pvc",
    "description": "放量事件(ma5/ma60 成交额>1.3) × 多尺度方向投票 × BTC regime，半衰期 10d 衰减",
}
...
def calc_factor(data_ctx: dict[str, pd.DataFrame]) -> pd.DataFrame:
    """Return the raw daily volume-surge trend-decay matrix (date x instrument)."""
    ...
    event_val = (dir_score * align * (0.5 + 0.5 * strength)).where(trigger)
