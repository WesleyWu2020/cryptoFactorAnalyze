"""Batch-evaluate all converted unsubmit factors (factor_common daily contract).

Runs every converted factor module under factor_analyse/unsubmit/{.,alpha191}
...
outputs/unsubmit_results.json for the summary notebook.

Factors that need data unavailable in the daily H5 (open_interest, mark/index
close, bybit standards, cross-venue share) are listed in SKIPPED and recorded
with status="skipped".

Usage:
    ./.venv/bin/python scripts/batch_evaluate_unsubmit.py [--workers 6] [--no-report]
"""
...
UNSUBMIT_DIR = PROJECT_ROOT / "factor_analyse" / "unsubmit"
FACTOR_DIRS = (UNSUBMIT_DIR, UNSUBMIT_DIR / "alpha191")

SKIPPED = {
    "crypto_alpha_basis_meanrev_1d": "需要 mark_close/index_close（日频 H5 未提供）",
    "crypto_alpha_bybit_heat_standard_tier_20_120": "需要 bybit standards 分级数据（日频 H5 未提供）",
    "crypto_alpha_comp_attention_burst_oi": "需要 open_interest（日频 H5 未提供）",
    "crypto_alpha_comp_attention_oi_buildup": "需要 open_interest（日频 H5 未提供）",
    "crypto_alpha_comp_oi_buildup_chase_int": "需要 open_interest/mark_close（日频 H5 未提供）",
    "crypto_alpha_oi_buildup_premium_20_120": "需要 open_interest（日频 H5 未提供）",
    "crypto_alpha_oi_buildup_premium_20_120_longfil": "需要 open_interest（日频 H5 未提供）",
    "crypto_alpha_oi_chase_intensity_premium_5d": "需要 open_interest/mark_close（日频 H5 未提供）",
    "crypto_alpha_oi_chase_reversal_5d": "需要 open_interest/mark_close（日频 H5 未提供）",
    "crypto_alpha_venue_share_shift_20_120": "跨交易所份额（bybit/binance），日频仅 binance 数据，份额退化为常数",
}
...
def main() -> int:
    ...
    paths = sorted(
        ...
        for p in d.glob("*.py")
        if p.stem not in SKIPPED
    )
    print(f"factors: {len(paths)}  skipped: {len(SKIPPED)}  workers: {args.workers}", flush=True)
    ...
    for stem, reason in SKIPPED.items():
        results.append({"factor": stem, "status": "skipped", "error": reason})
    ...
    failures = [r for r in results if r.get("status") not in ("complete", "incomplete", "skipped")]
    print(f"done: {len(results) - len(failures) - len(SKIPPED)} ok, {len(SKIPPED)} skipped, {len(failures)} failed")
