...
def test_candidate_banks_are_deterministic_and_defensive():
    ...
    assert [item["candidate_id"] for item in lightgbm] == [f"c{i:03d}" for i in range(6)]
    assert lightgbm[0]["params"] == {
        "num_leaves": 7,
        "min_data_in_leaf": 50,
        "lambda_l2": 1.0,
    }
    lightgbm[0]["params"]["num_leaves"] = 999
    assert candidate_bank("lightgbm", config)[0]["params"]["num_leaves"] == 7
    assert len(candidate_bank("xgboost", config)) == 6
    assert [item["params"]["alpha"] for item in candidate_bank("ridge", config)] == [
        0.0001,
        0.001,
        0.01,
        0.1,
    ...
    assert candidate_bank("linear", config) == [{"candidate_id": "c000", "params": {}}]
...
def test_require_backends_is_lazy_mapping():
    backends = require_backends(("lightgbm", "xgboost", "sklearn.linear_model"))
    assert set(backends) == {"lightgbm", "xgboost", "sklearn.linear_model"}
